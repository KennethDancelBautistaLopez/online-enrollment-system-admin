...
export default function ManageCurriculum() {
    ...
    const fetchCurricula = async () => {
      ...
      try {
        const res = await axios.get(
          "/api/subject-mappings?getAllCourseStructure=true"
        );
        if (res.data.error) {
  ...
  const handleDeleteSubject = async () => {
    ...
    try {
      await axios.delete(
        `/api/subject-mappings?subjectId=${subjectId}&curriculumId=${curriculumId}`
      );
      ...
      {loading ? (
        ...
        <>
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            <h2 className="text-3xl pt-5 font-bold mb-6 text-gray-900 dark:text-white text-center">
              Curriculum Guide
            </h2>
            ...
            <div className="space-y-4">
              {curricula.map((courseData) => (
                <div
                  ...
                  className="bg-gray-100 hover:bg-gray-100 dark:bg-gray-700 p-4 rounded-lg"
                >
                  <h3 className="text-xl font-bold mb-2">
                    {courseData.course} -{" "}
                    {courseDescriptions[courseData.course]}
                  </h3>
                  {courseData.yearLevels.map((yearData) => (
                    <div key={yearData.yearLevel} className="ml-4 mb-3">
                      <h4 className="text-lg font-semibold">
                        Year Level: {year[yearData.yearLevel]}
                      </h4>
                      {yearData.semesters.map((semesterData) => (
                        <div key={semesterData.semester} className="ml-6 mb-2">
                          <h5 className="text-md font-medium">
                            {semesterData.semester}{" "}
                            {/* <div className="float-right mx-2">Actions</div> */}
                          </h5>
                          <ul className="ml-4 list-disc">
                            {semesterData.subjects.map((subject) => (
                              <li
                                key={subject.code}
                                className="text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 border-b border-t dark:border-t-gray-600 dark:border-b-gray-600 dark:hover:bg-gray-600 py-1 hover:bg-gray-200 flex items-center justify-between"
                              >
            ...
            <div className="fixed inset-0 dark:bg-gray-900 dark:bg-opacity-50 flex items-center justify-center bg-gray-500 bg-opacity-75 z-50">
              <div className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow-md w-1/3">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white text-center mb-4">
                  ...
                  <span className="text-red-500">{confirmDelete.code}?</span>
                </h3>
