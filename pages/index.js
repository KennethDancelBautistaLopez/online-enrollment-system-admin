    ...
    const fetchEvents = async () => {
      try {
        ...
        data.forEach((event) => {
          ...
          if (
            eventDate < now &&
            !shownConflicts.current.has(event.title + "::past")
          ) {
            shownConflicts.current.add(event.title + "::past");

            toast(
              `The event "${
                event.title
              }" already passed on ${eventDate.toDateString()}.`,
              {
                icon: "📅",
                style: {
                  background: "#f8d7da",
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {events.slice(0, 3).map((event) => (
                ...
                >
                  ...
                  <div className="mb-2 ">
                    <p
                      className="text-gray-700 p-2 dark:border-gray-600  border-2 rounded-md border-blue-900  cursor-pointer dark:text-gray-300 dark:text-gray-400"
                  ...
                  <div className="text-sm text-gray-600 space-y-1 dark:text-gray-400">
                    ...
                    <p>
                      🎫 <span className="italic">{event.eventType}</span>
                    </p>
                  </div>
                </div>
              ))}
