...
export default async function handler(req, res) {
 ...
 if (req.method === 'POST') {
    ...
    try {
      ...
      let curriculum = await Curriculum.findOne({ course, yearLevel: yearLevelNumber, semester });
      ...
      if (curriculum) {
        ...
        for (let newSubject of subjects) {
          ...
          const duplicateSubject = curriculum.subjects.find(subject => subject.code === newSubject.code);
      ...
      curriculum = await Curriculum.create({ course, yearLevel: yearLevelNumber, semester, subjects });
  ...
  try {
    ...
    if (getAllCourseStructure === 'true') {
      ...
      for (const course of courses) {
        ...
        const yearLevelData = await Promise.all(yearLevels.map(async (yearLevel) => {
          // Get all semesters for each year level within the course
          const semesters = await Curriculum.distinct('semester', { course, yearLevel });
          ...
          const semesterData = await Promise.all(semesters.map(async (semester) => {
            ...
            const curriculum = await Curriculum.findOne({ course, yearLevel, semester });
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch course structure' });
  }
}
  ...
  if (!subjectId || !curriculumId) {
    return res.status(400).json({ success: false, error: 'Subject ID and Curriculum ID are required' });
  }
  ...
  try {
    ...
    if (!curricula.length) {
     ...
     for (const curriculum of curricula) {
      curriculum.subjects = curriculum.subjects.filter(subject => subject._id.toString() !== subjectId);
      await curriculum.save();
    }
